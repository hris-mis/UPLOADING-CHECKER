...
(() => {
    ...
    const showUncheckedToggle = $('#showUncheckedToggle');
    ...
    const workInput = $('#workScheduleInput');
    const restInput = $('#restScheduleInput');
    const workTableBody = $('#workTableBody');
    const restTableBody = $('#restTableBody');
    const summaryEl = $('#summary');
    const warningBanner = $('#warning-banner');
    const successMsg = $('#success-message');
    const generateWorkFileBtn = $('#generateWorkFile');
    const generateRestFileBtn = $('#generateRestFile');
    const clearWorkBtn = $('#clearWorkData');
    const clearRestBtn = $('#clearRestData');
    const tabSchedule = $('#tab-schedule');
    const tabMonitoring = $('#tab-monitoring');
    const scheduleContent = $('#tab-schedule-content');
    const monitoringContent = $('#tab-monitoring-content');
    // Monitoring refs
    const monitoringBody = $('#monitoringBody');
    const clearMonitoringBtn = $('#clearMonitoringBtn');
    const exportMonitoringBtn = $('#exportMonitoringBtn');
    const monthSelect = $('#monthSelect');
    const yearSelect = $('#yearSelect');
    const totalBranchesEl = $('#totalBranches');
    const checkedBranchesEl = $('#checkedBranches');
    const uploadedBranchesEl = $('#uploadedBranches');
    ...
    function detectHeaderAndMap(rows) {
        ...
        const isDateLike = (v) => /^\d{1,2}[\/\.\-]\d{1,2}[\/\.\-]\d{2,4}$/.test(v) || /^\d{5}$/.test(v) || /^\d{4}-\d{2}-\d{2}$/.test(v);
        const isShift = (v) => /am|pm|to|-|–|:/.test(v) && /\d/.test(v);
        ...
        for (let i = 0; i < rows.length; i++) {
            ...
            if (row.some((c) => /name|fullname|employee|branch|position|shift|day/.test(c))) {
                headerIndex = i;
                break;
            }
        }
        ...
        if (headerIndex === -1) {
            ...
            if (empIdx !== -1)
                mapping.empNo = empIdx;
            if (dateIdx !== -1)
                mapping.date = dateIdx;
            if (shiftIdx !== -1)
                mapping.shift = shiftIdx;
            if (dayIdx !== -1)
                mapping.day = dayIdx;
    ...
    function handlePaste(e, type) {
        ...
        if (branchName) {
            const branchInput = type === 'work' ? $('#workBranchName') : $('#restBranchName');
    ...
    function undoDelete(type) {
        ...
        if (!stack || !stack.length)
            return showBanner('Nothing to undo.');
    ...
    function redoPaste(type) {
        ...
        if (!stack || !stack.length)
            return showBanner('Nothing to redo.');
        const snap = stack.pop();
        undoStack[type].push({ work: JSON.parse(JSON.stringify(workScheduleData)), rest: JSON.parse(JSON.stringify(restDayData)) });
        workScheduleData = snap.work;
        restDayData = snap.rest;
    ...
    function showRejectedModal(rejected) {
        ...
        if (title)
            title.textContent = 'Smart Paste Summary';
    ...
    if (clearWorkBtn)
        clearWorkBtn.addEventListener('click', () => {
            ...
            renderWorkTable();
            updateButtonStates();
            showBanner('Work schedule cleared.');
            saveState();
        });
    if (clearRestBtn)
    ...
    function updateMonitoringStats() {
        ...
        if (checkedBranchesEl)
            checkedBranchesEl.textContent = String(checked);
        if (uploadedBranchesEl)
            uploadedBranchesEl.textContent = String(uploaded);
    ...
    const monitorFilterUnchecked = $('#filterUnchecked');
    if (monitorSearchInput)
        monitorSearchInput.addEventListener('input', () => renderMonitoring());
    if (monitorFilterUnchecked)
        monitorFilterUnchecked.addEventListener('change', () => { showUnchecked = !!monitorFilterUnchecked.checked; renderMonitoring(); });
    ...
    (function insertClearAll() {
        ...
        btn.addEventListener('click', () => {
            if (!confirm('Are you sure you want to clear ALL schedule data (Work + Rest)?'))
                return;
            workScheduleData = [];
            restDayData = [];
            rejectedRows = [];
            ...
            if (restInput)
                restInput.value = '';
            const wbEl = $('#workBranchName');
            const rbEl = $('#restBranchName');
            if (wbEl)
                wbEl.value = '';
            if (rbEl)
                rbEl.value = '';
