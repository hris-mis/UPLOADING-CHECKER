"use strict";
/**
 * Cleaned and TypeScript-optimized version of the original main.js
 * Converted, typed, and formatted — all original features preserved.
 * Author: HRIS-MIS Team
 * Last updated: 2025-10-18
 */
/*
Converted TypeScript (best-effort) from original main.js
- Original JS included below for reference.
- This TS file uses DOM types and simple annotations so you can compile with tsc.
- I also included the original main.js (unchanged) in the zip so the page still runs if you don't compile the TS.
// ----- original main.js (shortened snippet) -----
...
function detectColumnMapping(rows) {
    ...
    const validRows = dataRows.filter(r => {
        ...
        if (!colMap.empNo && r.length > 0) {
            ...
            if (possibleEmp)
                colMap.empNo = r.indexOf(possibleEmp);
        }
    ...
    function handlePaste(e, type) {
        ...
        rows.forEach((row) => {
            ...
            if (obj.empNo)
                obj.empNo = obj.empNo.replace(/[^0-9]/g, '').trim();
        ...
        function renderMonitoring() {
            var _a;
            const data = getMonitoring();
            const searchVal = ((_a = (document.getElementById('monitorSearch') || {}).value) === null || _a === void 0 ? void 0 : _a.trim().toLowerCase()) || '';
        ...
        if (exportMonitoringBtn) {
            exportMonitoringBtn.addEventListener('click', () => {
                const data = getMonitoring();
                if (!data.length)
                    return showBanner('⚠️ No monitoring data to export.');
                const month = (monthSelect === null || monthSelect === void 0 ? void 0 : monthSelect.value) || 'N/A';
                const year = (yearSelect === null || yearSelect === void 0 ? void 0 : yearSelect.value) || new Date().getFullYear();
                const total = data.length;
                const uploaded = data.filter(b => b.uploaded).length;
        ...
        if (monitorFilterUnchecked)
            monitorFilterUnchecked.addEventListener('change', () => renderMonitoring());
    ...
    ;
    (() => {
        // Strict typing for DOM and utility helpers
